Shader "Stylized/SkyMine"
{
    Properties
    {
        ...
        [Header(Stars)]
        _StarsColor ("Color", Color) = (0.9044118, 0.8872592, 0.7913603, 1)
        _StarsPower ("Power", float) = 4
        _StarsTexture ("Noise", 2D) = "white" {}
        _StarsAmount ("Amount", float) = 20
    }
    SubShader
    {
        Tags
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float3 temp = normalize(i.worldPosition);
                float2 tempuv = float2(temp[0],temp[2]);
                tempuv /= temp[1];



                float4 stars = tex2D(_StarsTexture, tempuv * _StarsAmount);
                float3 starsColor = pow(stars, _StarsPower);

                starsColor = starsColor * _StarsColor;
                
               // stars = pow(_StarsTexture, _StarsPower);


                float3 finalColor = lerp(saturate(sunHaloColor + horizonLineColor + skyGradientColor + starsColor), _SunDiscColor, maskSun);
